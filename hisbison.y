...
stat:E SC stat               {$$.code = code_concatenate(2,$1.code,$3.code); $$.optimized_code = code_concatenate(2,$1.optimized_code, $3.optimized_code);}
    ...
    |				{$$.code = strdup(" "); $$.optimized_code = $$.code;}
    ;
...
ST  : SWITCH OPEN ID CLOSE OPBRACE {scope_ctr++;scope[scope_ind++]=scope_ctr;} B CLBRACE 
											{
												scope[scope_ind++]=0;
												if(!look_up_sym_tab($3)){printf("19     Undeclared variable %s\n", $3); YYERROR;}
...
iter_stat:FOR OPEN decl cond SC E CLOSE comp_stat		{char* begin = new_label(); 
															...
															$$.code = code_concatenate(8,$3.code, code_gen(2,begin,": "),code_gen(4,"if ", $4.code, "goto ", $4.true1), 
															code_gen(2,"goto ", $4.false1), code_gen(2,$4.true1,": "),$8.code, code_gen(2,"goto ",begin),code_gen(2,$8.next,": \n"));
...
assign_expr:ID ASSIGN E COMMA assign_expr     {$$.addr = gen_addr($1);  
										...
										if($3.is_dig)
			   							...
			   							if(!look_up_sym_tab($1)){printf("24     Undeclared variable %s\n", $1); YYERROR;}flag1=1;
			   							}
		   |ID ASSIGN E SC            	{
		  								$$.addr = gen_addr($1); 
		  								int scop=get_scope(); 
										...
										if($3.is_dig)
										{
			   							...
			   							//printf("OC: %s\n",$$.optimized_code);
			   							if(!look_up_sym_tab($1)){printf("25     Undeclared variable %s\n", $1); YYERROR;}
			   							flag1=1;												
			   							}
		   ;
...
F:ID  {$$.addr = gen_addr($1); $$.code = code_gen(1," ");if(!look_up_sym_tab($1)){printf("26        Undeclared variable %s\n", $1); YYERROR;} $$.optimized_code = $$.code;$$.is_dig=0;}
...
s_operation: ID s_op ID  {$$.addr = new_temp(); 
								...
								if(!look_up_sym_tab($1)){printf("27     Undeclared variable %s\n", $1); YYERROR;}
								if(!look_up_sym_tab($3)){printf("28     Undeclared variable %s\n", $3); YYERROR;}
								$$.optimized_code = $$.code;
								$$.is_dig=0;
								}
			   | ID s_op NUM {$$.addr = new_temp(); 
								$$.code = code_concatenate(2,code_gen(5,$$.addr,"=",$1,$2,$3), code_gen(3,$1,"=",$$.addr)); 
								if(!look_up_sym_tab($1)){printf("29     Undeclared variable %s\n", $1); YYERROR;}
								$$.optimized_code = $$.code;
								$$.is_dig=0;
								}
			   | ID s_op OPEN E CLOSE {$$.addr = new_temp(); 
								$$.code = code_concatenate(3,$4.code,code_gen(5,$$.addr,"=",$1,$2,$4.addr), code_gen(3,$1,"=",$$.addr)); 
								if(!look_up_sym_tab($1)){printf("30     Undeclared variable %s\n", $1); YYERROR;}
								$$.optimized_code = $$.code;
								$$.is_dig=0;
								}
			   ;
...
unary_expr:INC ID 			{ $$.addr = new_temp(); 
								...
								if(!look_up_sym_tab($2)){printf("31     Undeclared variable %s\n", $2); YYERROR;}
								$$.optimized_code = $$.code;
								$$.is_dig=0;
							}
		  |ID INC  			{ $$.addr = new_temp(); 
								$$.code = code_concatenate(2,code_gen(4,$$.addr,"=",$1,"+ 1"),code_gen(3,$1,"=",$$.addr));
								if(!look_up_sym_tab($1)){printf("32     Undeclared variable %s\n", $1); YYERROR;}
								$$.optimized_code = $$.code;
								$$.is_dig=0;
							}
		  |DEC ID           {  $$.addr = new_temp(); 
								$$.code = code_concatenate(2,code_gen(4,$$.addr,"=",$2,"- 1"),code_gen(3,$2,"=",$$.addr));
								if(!look_up_sym_tab($2)){printf("33     Undeclared variable %s\n", $2); YYERROR;}
								$$.optimized_code = $$.code;
								$$.is_dig=0;
							}
		  |ID DEC 			{  $$.addr = new_temp(); 
								$$.code = code_concatenate(2,code_gen(4,$$.addr,"=",$1,"- 1"),code_gen(3,$1,"=",$$.addr));
								if(!look_up_sym_tab($1)){printf("34     Undeclared variable %s\n", $1); YYERROR;}
								$$.optimized_code = $$.code;
								$$.is_dig=0;
							}
		  | MINUS ID       {  $$.addr = new_temp(); 
								$$.code = code_gen(4,$$.addr,"=",$1,$2); 
								if(!look_up_sym_tab($1)){printf("35     Undeclared variable %s\n", $1); YYERROR;}
								$$.optimized_code = $$.code;
								$$.is_dig=0;
							}
